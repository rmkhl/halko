...
func (p *Program) validateStepOrder() error {
	...
	for i := 1; i < len(p.ProgramSteps); i++ {
		...
		// Skip runtime-only steps (that don't have a temperature target)
		if currentStep.TargetTemperature == 0 || previousStep.TargetTemperature == 0 {
			continue
		}
