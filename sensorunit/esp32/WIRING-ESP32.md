...
```text
            ...
            D32  ───┤ D32             D19 ├───  GPIO19 (VSPI MISO) ★
            D33  ───┤ D33             D18 ├───  GPIO18 (VSPI SCK)  ★
            D25  ───┤ D25              D5 ├───  GPIO5  (CS #1)     ★
            D26  ───┤ D26             TX2 ├───  GPIO17 (CS #2)     ★
            D27  ───┤ D27             RX2 ├───  GPIO16 (CS #3)     ★
            D14  ───┤ D14              D4 ├───  GPIO4
            D12  ───┤ D12              D2 ├───  GPIO2
            D13  ───┤ D13             D15 ├───  GPIO15
            GND  ───┤ GND             GND ├───  GND
...
| Kiln Secondary | GPIO17 | TX2 | Module 2 CS pin | Kiln thermocouple #2 |
| Wood Material | GPIO16 | RX2 | Module 3 CS pin | Wood thermocouple |

**Note:** On the 30-pin DevKit V1, GPIO17 and GPIO16 are silkscreened **TX2**
and **RX2** (some 38-pin boards label them D17/D16).
...
All three MAX31855 modules share the 3.3V, GND, SCK, and DO (MISO) buses;
each module has its own chip-select line. The OLED display shares 3.3V/GND
and uses the separate I2C pins.
...
```text
        ...
        └─┬────┬────┬────┬────┬────┬────┬────┬────┬┘
         3V3  GND  D18  D19  D5   TX2  RX2  D21  D22
          │    │   SCK  MISO CS1  CS2  CS3  SDA  SCL
          │    │    │    │    │    │    │    │    │
          │    │    │    │    │    │    │    │    │  ┌───────────────────────────┐
          ●────┼────┼────┼────┼────┼────┼────┼────┼──┤ VIN     MAX31855 #1       │
          │    ●────┼────┼────┼────┼────┼────┼────┼──┤ GND    (Kiln Primary)     │
 ...
 ● = connection tap (wire continues)    ┼ = wires crossing (not connected)
```
...
| **MAX31855 #2** | CS | GPIO17 | TX2 | Individual chip select |
| **MAX31855 #3** | CS | GPIO16 | RX2 | Individual chip select |
