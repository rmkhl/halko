...
func handleNginxCommand() {
	...
	executorHost, executorPort, err := extractHostPort(config.APIEndpoints.Executor.URL)
	if err != nil {
		fmt.Fprintf(os.Stderr, "Error: Failed to parse executor URL: %v\n", err)
		os.Exit(exitError)
	}
	...
	storageHost, storagePort, err := extractHostPort(config.APIEndpoints.Storage.URL)
	if err != nil {
		fmt.Fprintf(os.Stderr, "Error: Failed to parse storage URL: %v\n", err)
		os.Exit(exitError)
	}

	nginxConfig := generateNginxConfig(*port, executorHost, executorPort, sensorHost, sensorPort, powerHost, powerPort, storageHost, storagePort)
...
func generateNginxConfig(listenPort int, executorHost, executorPort, sensorHost, sensorPort, powerHost, powerPort, storageHost, storagePort string) string {
...
`, listenPort, executorHost, executorPort, sensorHost, sensorPort, powerHost, powerPort, storageHost, storagePort)
}
