...
func handleNginxCommand() {
	...
	dbusHost, dbusPort, err := extractHostPort(config.APIEndpoints.DBusUnit.URL)
	if err != nil {
		fmt.Fprintf(os.Stderr, "Error: Failed to parse dbusunit URL: %v\n", err)
		os.Exit(exitError)
	}

	nginxConfig := generateNginxConfig(*port, controlunitHost, controlunitPort, sensorHost, sensorPort, powerHost, powerPort, dbusHost, dbusPort)
...
func generateNginxConfig(listenPort int, controlunitHost, controlunitPort, sensorHost, sensorPort, powerHost, powerPort, dbusHost, dbusPort string) string {
...
server {
    ...
    # Proxy for dbusunit service (VPN and power management)
    location /api/v1/dbusunit/ {
        proxy_pass http://%s:%s/;
        proxy_http_version 1.1;
        proxy_set_header Host $host;
        proxy_set_header X-Real-IP $remote_addr;
